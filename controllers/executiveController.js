...
//       // Background: Handle potential Cloudinary DNS blips
...
// // ... keep update and delete as they were, but ensure they return { success, data }
const Executive = require('../models/Executive');
const cloudinary = require('cloudinary').v2;
...
exports.updateExecutive = async (req, res) => {
  try {
    ...
    if (updateData.isGroupPhoto !== undefined) updateData.isGroupPhoto = String(updateData.isGroupPhoto) === 'true';
    ...
    const updatedExecutive = await Executive.findByIdAndUpdate(id, updateData, { returnDocument: 'after', runValidators: true });
    return res.status(200).json({ success: true, message: 'Updated successfully.', data: updatedExecutive });
  } catch (err) {
